...
static uint8_t ucAzureIoTADURootKeyN200702[385] = {0x00, 0xd5, 0x42, 0x2e, 0xaf, 0x11, 0x54, 0xa3, 0x50, 0x65, 0x87, 0xa2, 0x4d, 0x5b, 0xba,
...
static uint8_t ucAzureIoTADURootKeyN200703[385] = {0x00, 0xb2, 0xa3, 0xb2, 0x74, 0x16, 0xfa, 0xbb, 0x20, 0xf9, 0x52, 0x76, 0xe6, 0x27, 0x3e,
...
static SampleJWS::RootKey xADURootKeys[] = {{// Minus one on id to not count NULL
...
float t, h;
bool is_dht_measument_valid = false;

static unsigned long next_sensors_measurements_ms = 0;
static int motion_detected = 0;
static int motion_measurements = 0;
...
bool getTemperatureAndHumidity()
{
  ...
  h = dht.readHumidity();
  return !isnan(t) && !isnan(h) && h >= 0 && h <= 100;
}
...
static void request_all_properties(void)
{
  ...
  if (esp_mqtt_client_publish(
          mqtt_client, property_document_topic_buffer, NULL, 0, MQTT_QOS1, DO_NOT_RETAIN_MSG) < 0)
  {
...
{
  return (
      (az_span_size(adu_update_manifest.instructions.steps[0].handler_properties.installed_criteria) == sizeof(ADU_DEVICE_VERSION) - 1) && strncmp(
                                                                                                                                               ADU_DEVICE_VERSION,
                                                                                                                                               (char *)az_span_ptr(
                                                                                                                                                   adu_update_manifest.instructions.steps[0].handler_properties.installed_criteria),
                                                                                                                                               az_span_size(
                                                                                                                                                   adu_update_manifest.instructions.steps[0].handler_properties.installed_criteria)) == 0);
}
    ...
    az_iot_hub_client_properties_message_type message_type)
{
  ...
  az_span scratch_buffer_span = az_span_create((uint8_t *)adu_scratch_buffer, (int32_t)sizeof(adu_scratch_buffer));
...
az_span az_span_ftoa(az_span destination, float value, int decimal_places)
{
  char buffer[32];
  dtostrf(value, (decimal_places + 3), decimal_places, buffer); // Convert float to string
  return az_span_copy(destination, az_span_create_from_str(buffer));
}
...
static void get_telemetry_payload(az_span payload, az_span *out_payload)
{
  ...
  payload = az_span_ftoa(payload, t, 2); // Convert temperature to string with 2 decimal places
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", \"humidity\": "));
  payload = az_span_ftoa(payload, h, 2); // Convert humidity to string with 2 decimal places
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", \"motion_detected\": "));
  rc = az_span_u32toa(payload, motion_detected, &payload);
  (void)rc;
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(", \"motion_measurements\": "));
  rc = az_span_u32toa(payload, motion_measurements, &payload);
  (void)rc;
  payload = az_span_copy(payload, AZ_SPAN_FROM_STR(" }"));
...
void loop()
{
  ...
  if (millis() > next_sensors_measurements_ms)
  {
    // Read DHT sensor
    is_dht_measument_valid = getTemperatureAndHumidity();

    // Read motion sensor
    int motion_signal = digitalRead(MOTION_SENSOR_PIN);
    ...
    next_sensors_measurements_ms = millis() + SENSORS_DELAY;
  }
  ...
  else if (millis() > next_telemetry_send_time_ms)
  {
    ...
    // if the last temperature and humidity measurement is valid send telemetry
    if (is_dht_measument_valid)
    {
      send_telemetry();
      motion_detected = 0;
      motion_measurements = 0;
      Serial.println("OK");
    }
