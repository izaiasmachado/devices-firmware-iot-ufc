...
{
  // Buffer for the telemetry payload
  ...
  // Initialize the az_span to manage the telemetry payload buffer
  az_span temp_span = az_span_create((uint8_t *)telemetry_payload, sizeof(telemetry_payload));

  // Construct JSON payload
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR("{ \"temperature\": "));
  temp_span = az_span_ftoa(temp_span, t, 2); // Convert temperature to string with 2 decimal places
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \"humidity\": "));
  temp_span = az_span_ftoa(temp_span, h, 2); // Convert humidity to string with 2 decimal places
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \"motion_detected\": "));
  (void)az_span_u32toa(temp_span, motion_detected, &temp_span);
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \"motion_measurements\": "));
  (void)az_span_u32toa(temp_span, motion_measurements, &temp_span);
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(" }"));

  // Null-terminate the string
  temp_span = az_span_copy_u8(temp_span, '\0');

  // Print the telemetry payload
