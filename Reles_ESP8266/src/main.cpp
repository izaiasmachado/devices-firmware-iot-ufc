...
void setDoorState(char *payload);
void setLightState(char *payload);
...
void receivedCallback(char *topic, byte *payload, unsigned int length)
{
  ...
  Serial.println("Changing door and light state");
  Serial.println((char)payload[0]);
  Serial.println((char)payload[1]);
  setDoorState((char *)payload);
  setLightState((char *)payload);
}

void setDoorState(char *payload)
{
  if (payload[0] != '1' && payload[0] != '0' && payload[0] != '2')
  {
    return;
  }
  ...
  if (payload[0] == '2')
  {
    if (digitalRead(DOOR_RELAY_PIN) == LOW)
    {
      return;
    }
  ...
  digitalWrite(DOOR_RELAY_PIN, payload[0] - 48);
}
...
void setLightState(char *payload)
{
  ...
  digitalWrite(LIGHT_RELAY_PIN, payload[1] - 48);
}
